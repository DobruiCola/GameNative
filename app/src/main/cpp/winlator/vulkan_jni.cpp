...
#include <cstdlib>
#include <cstring>
// Use the angle-bracket form so the CMake include path resolves correctly.
// (adrenotools is at extras/adrenotools/ in GameNative, not adjacent to winlator/)
    ...
    JNIEnv*, jobject, jlong handle, jlong ahbPtr, jint x, jint y, jint w, jint h, jint fenceFd)
{
    auto* r = reinterpret_cast<VulkanRendererContext*>(handle);
    if (r && ahbPtr) {
        r->scanoutSetBuffer(reinterpret_cast<AHardwareBuffer*>(ahbPtr), x, y, w, h, (int)fenceFd);
    } else if (fenceFd >= 0) {
        ::close((int)fenceFd);
    }
}
...
extern "C" JNIEXPORT void JNICALL
Java_com_winlator_renderer_VulkanRenderer_nativeSetColorAdjustment(JNIEnv*, jobject, jlong handle, jfloat brightness, jfloat contrast, jfloat gamma) {
    auto* r = reinterpret_cast<VulkanRendererContext*>(handle);
    if (r) r->setColorAdjustment((float)brightness, (float)contrast, (float)gamma);
}
